...
namespace IFix.Core
{
    ...
    public class NewFieldInfo
    {
        ...
        static readonly Dictionary<int, Dictionary<string, object>> newFieldValues =
    ...
    unsafe public class VirtualMachine
    {
        ...
        {
            get { return externTypes; }
            set { externTypes = value; }
        }

        public MethodBase[] ExternMethods
        {
            get { return externMethods; }
            set
            {
                externMethods = value;
        ...
        public FieldInfo[] FieldInfos
        {
            ...
            {
                fieldInfos = value;
                fieldTypes = new Type[value.Length];
                fieldDeclareType = new Type[value.Length];
            }
        }

        public Dictionary<int, NewFieldInfo> NewFieldInfos
        {
            get { return newFieldInfos; }
            set { newFieldInfos = value; }
        }

        public AnonymousStoreyInfo[] AnonymousStoreyInfos
        {
            get { return anonymousStoreyInfos; }
            set { anonymousStoreyInfos = value; }
        }

        public Type[] StaticFieldTypes
        {
            get { return staticFieldTypes; }
            set
            {
        ...
        {
            get { return wrappersManager; }
            set { wrappersManager = value; }
        }

        public delegate bool ExternInvokersHandle(MethodBase mb, out ExternInvoker ei);

        public ExternInvokersHandle externInvokersHandle;
        ...
        void store(Value* stackBase, Value* dst, Value* src, object[] managedStack)
        {
            *dst = *src;
            switch (dst->Type)
            {
                ...
                {
                    BoxUtils.RecycleObject(managedStack[dst - stackBase]); //field addr
                    managedStack[dst - stackBase] = managedStack[src - stackBase];

                    int srcIndex = (int)(src - stackBase);
                ...
                {
                    var obj = managedStack[src->Value1];
            ...
            while (true) //TODO: 常用指令应该放前面
            {
                try
                {
                    ...
                    switch (code)
                    {
                        ...
                        case Code.Ldarg:
                        {
                            var src = argumentBase + pc->Operand;
                        ...
                        case Code.CallExtern: //部分来自Call部分来自Callvirt
                        {
                            ...
                            stack.UnmanagedStack->Top = evaluationStackPointer;
                            externInvokeFunc(this, ref call, false);
                        ...
                        case Code.Newobj: // 2.334642%
                        {
                            ...
                            //调用外部前，需要保存当前top，以免外部从新进入内部时覆盖栈
                            stack.UnmanagedStack->Top = evaluationStackPointer;
                            externInvokeFunc(this, ref call, true);
                            evaluationStackPointer = call.currentTop;
                        }
                            break;
                        ...
                        case Code.Stloc:
                        {
                            ...
                            var dst = localBase + pc->Operand;
                            *dst = *evaluationStackPointer;
                            switch (dst->Type)
                            {
                                case ValueType.ChainFieldReference:
                                {
                        ...
                        case Code.Ldfld: //5.017799%
                        {
                            ...
                            if (fieldIndex >= 0)
                            {
                                ...
                                else
                                {
                                    ...
                                    {
                                        fieldTypes[fieldIndex] = fieldInfo.FieldType;
                                    }

                                    if (fieldDeclareType[fieldIndex] == null)
                                    {
                                        fieldDeclareType[fieldIndex] = fieldInfo.DeclaringType;
                                    }

                                    fieldType = fieldTypes[fieldIndex];
                                    declaringType = fieldDeclareType[fieldIndex];
                                }
                                ...
                                if (obj == null)
                                {
                                    throw new NullReferenceException(declaringType + "." + fieldInfo != null
                                        ? fieldInfo.Name
                                        : newFieldInfos[fieldIndex].Name);
                                }
                                ...
                                {
                                    EvaluationStackOperation.PushField(evaluationStackBase, ptr, managedStack, obj,
                                        fieldInfo, fieldType);
                                ...
                                // EvaluationStackOperation.PushObject(evaluationStackBase, ptr, managedStack, fieldValue, fieldType);
                            }
                            else
                            {
                        ...
                        {
                            evaluationStackPointer--;
                            bool transfer = evaluationStackPointer->Value1 == 0;
                        ...
                        case Code.Castclass: //0.358122%
                        {
                            ...
                            if (obj != null)
                            {
                                ...
                                if (canAssign && pc->Operand < 0 && (obj is AnonymousStorey) &&
                                    (obj as AnonymousStorey).typeId != -(pc->Operand + 1))
                                {
                                    var fromInfo = anonymousStoreyInfos[(obj as AnonymousStorey).typeId];
                                    var targetInfo = anonymousStoreyInfos[-(pc->Operand + 1)];

                                    if (fromInfo.Slots != null && targetInfo.Slots != null &&
                                        fromInfo.Slots.Length == targetInfo.Slots.Length)
                                    {
                                    ...
                                    if (!canAssign)
                                    {
                                        throw new InvalidCastException("AnonymousStorey typeid different, " +
                                                                       (obj as AnonymousStorey).typeId + " <-> " +
                                                                       -(pc->Operand + 1));
                                    }
                                }
                            }
                        ...
                        case Code.Endfinally: //0.2513792%
                        {
                            if (leavePoint == 0) //有异常
                            {
                                ...
                                var exception = managedStack[(evaluationStackPointer - 1)->Value1]
                                    as Exception;
                                // BoxUtils.RecycleObject(managedStack[exceptionPos]); 不是 value类型
                        ...
                        case Code.Ldflda: //0.240527%
                        {
                            ...
                            {
                                object obj = EvaluationStackOperation.ToObject(evaluationStackBase, ptr,
                                    managedStack, pc->Operand < 0
                                        ? typeof(AnonymousStorey)
                        ...
                        case Code.Conv_U: //0.1764814% Convert to unsigned native int, pushing native int on stack.
                        {
                            ...
                            switch (ptr->Type)
                            {
                                ...
                                case ValueType.StackReference:
                                    val = (*(IntPtr*)(&ptr->Value1)).ToPointer();
                                    break;
                                default:
                                    val = null;
                            ...
                            switch (ptr->Type)
                            {
                                ...
                                default:
                                    throwRuntimeException(new InvalidProgramException(code
                                        + " expect ref, but got " + ptr->Type + " value1:" + ptr->Value1 + " value2:" +
                                        ptr->Value2), true);
                                    break;
                            }
                        }
                            ...
                            if (pc->Operand >= 0)
                            {
                                ...
                                if (ptr->Type == ValueType.Object)
                                {
                                    if (BoxUtils.GetTypeIsValueType(type))
                                    {
                                        ...
                                        {
                                            EvaluationStackOperation.UnboxPrimitive(ptr, obj,
                        ...
                        case Code.Constrained: //0.04714472% 
                        {
                            ...
                            var obj = EvaluationStackOperation.ToObject(evaluationStackBase, ptr, managedStack, type,
                                this);
                            var pos = (int)(ptr - evaluationStackBase);
                            BoxUtils.RecycleObject(managedStack[pos]);
                        ...
                        case Code.Newanon:
                        {
                            ...
                            AnonymousStorey anonymousStorey = (anonymousStoreyInfo.Slots == null)
                                ? new AnonymousStorey(anonymousStoreyInfo.FieldNum, anonymousStoreyInfo.FieldTypes,
                                    pc->Operand, anonymousStoreyInfo.VTable, this)
                                : wrappersManager.CreateBridge(anonymousStoreyInfo.FieldNum,
                                    anonymousStoreyInfo.FieldTypes, pc->Operand, anonymousStoreyInfo.VTable,
                        ...
                        case Code.Conv_Ovf_U8_Un:
                        {
                            ...
                            {
                                case ValueType.Long:
                                    val = pc->Code == Code.Conv_Ovf_U8
                                        ? checked((ulong)*(long*)&ptr->Value1)
                        ...
                        case Code.Conv_Ovf_I8_Un:
                        {
                            ...
                            {
                                case ValueType.Long:
                                    val = pc->Code == Code.Conv_Ovf_I8
                                        ? checked(*(long*)&ptr->Value1)
                        ...
                        case Code.Conv_Ovf_I1_Un:
                        {
                            ...
                            switch (ptr->Type)
                            {
                                case ValueType.Long:
                                case ValueType.Integer:
                                    val = pc->Code == Code.Conv_Ovf_I1
                                        ? checked((sbyte)ptr->Value1)
                        ...
                        case Code.Conv_Ovf_U2_Un:
                        {
                            ...
                            {
                                case ValueType.Long:
                                    val = pc->Code == Code.Conv_Ovf_U2
                                        ? checked((ushort)*(long*)&ptr->Value1)
                                        : checked((ushort)*(ulong*)&ptr->Value1);
                                    break;
                                case ValueType.Integer:
                                    val = pc->Code == Code.Conv_Ovf_U2
                                        ? checked((ushort)ptr->Value1)
                        ...
                        case Code.Conv_Ovf_U1_Un:
                        {
                            ...
                            {
                                case ValueType.Long:
                                    val = pc->Code == Code.Conv_Ovf_U1
                                        ? checked((byte)*(long*)&ptr->Value1)
                                        : checked((byte)*(ulong*)&ptr->Value1);
                                    break;
                                case ValueType.Integer:
                                    val = pc->Code == Code.Conv_Ovf_U1
                                        ? checked((byte)ptr->Value1)
                        ...
                        case Code.Conv_Ovf_U4_Un:
                        {
                            ...
                            {
                                case ValueType.Long:
                                    val = pc->Code == Code.Conv_Ovf_U4
                                        ? checked((uint)*(long*)&ptr->Value1)
                        ...
                        case Code.Conv_Ovf_I2_Un:
                        {
                            ...
                            switch (ptr->Type)
                            {
                                case ValueType.Long:
                                case ValueType.Integer:
                                    val = pc->Code == Code.Conv_Ovf_I2
                                        ? checked((short)ptr->Value1)
